...
const galleryData = {
  "2025": {
    Navratri: importAll(
      require.context("../assets/Events/Gallery/2025/Navratri/", false, /\.(jpg|jpeg|png)$/)
    ),
    Picnic: importAll(
      require.context("../assets/Events/Gallery/2025/Picnic/", false, /\.(jpg|jpeg|png)$/)
    ),
  },
  "2024": {
    Navratri: importAll(
      require.context("../assets/Events/Gallery/2024/Navratri/", false, /\.(jpg|jpeg|png)$/)
    ),
  },
};
...
export default function Gallery() {
  ...
  // Categories depend on selected year
  const categories =
    selectedYear === "All Years"
      ? ["All Categories"]
      : ["All Categories", ...Object.keys(galleryData[selectedYear])];

  // Function to get images based on filters
  const getFilteredImages = () => {
    ...
    if (selectedCategory === "All Categories") {
      Object.entries(galleryData[selectedYear]).forEach(([category, imgs]) => {
        imgs.forEach((img) => images.push({ img, category, year: selectedYear }));
      });
      return images;
    }

    galleryData[selectedYear][selectedCategory].forEach((img) => {
      images.push({ img, category: selectedCategory, year: selectedYear });
    });
  ...
  const showPrev = () => {
    setSelectedImageIndex(
      (selectedImageIndex - 1 + displayedImages.length) % displayedImages.length
    );
  };
        ...
        >
          ...
          >
            ...
            <div className="absolute bottom-3 left-1/2 -translate-x-1/2 bg-black/70 text-white text-sm px-3 py-1 rounded-full">
              {displayedImages[selectedImageIndex].year} • {displayedImages[selectedImageIndex].category}
            </div>
